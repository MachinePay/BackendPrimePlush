...
app.post("/api/payment/pix-digital", async (req, res) => {
  ...
  if (!MP_ACCESS_TOKEN) {
    return res.status(500).json({ error: "Access token Mercado Pago não configurado" });
  }
  try {
    const payload = {
      ...
      payer: { email: email || "cliente@exemplo.com" }
    };
    ...
    if (!response.ok) {
      return res.status(response.status).json({ error: data.message || "Erro ao criar pagamento PIX", details: data });
    }
    ...
    return res.json({ qrCode, qrCodeBase64, paymentId });
  } catch (error) {
    console.error("Erro ao criar PIX digital:", error);
