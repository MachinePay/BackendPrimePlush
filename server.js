  ...
  try {
    ...
    if (!Array.isArray(orderIds) || orderIds.length === 0) {
      return res.status(400).json({
        error: "Nenhum pedido enviado para recebimento.",
      });
    }

    // Buscar detalhes dos pedidos enviados
    ...
    console.log(
      `✅ Super Admin marcou R$ ${totalBrutoReceber.toFixed(2)} como recebido por IDs`,
    );
    ...
    console.error("❌ Erro ao marcar como recebido por IDs:", error);
    res.status(500).json({
      error: "Erro ao registrar recebimento por IDs",
